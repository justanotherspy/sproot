...
| 24 | Release pipeline: `homebrew_casks` block publishes a `sproot` cask to `justanotherspy/homebrew-tap` (`brew install --cask justanotherspy/tap/sproot`; darwin+linux, quarantine-strip postflight, needs a `HOMEBREW_TAP_GITHUB_TOKEN` PAT); release flow changed to prerelease-then-promote (`release.prerelease: true` + a `gh release edit --latest` step in `release.yml`) so the "Latest" badge only lands after every artifact (binaries, checksums, sigstore bundle, cask) is attached. One-time manual setup: create the `homebrew-tap` repo and add the PAT secret (done) |
